    ...
    <>
      <aside className="panel fixed left-4 top-4 z-20 hidden h-[calc(100vh-2rem)] w-64 overflow-hidden rounded-[28px] border border-white/10 bg-slate-950 text-white shadow-[0_30px_80px_rgba(15,23,42,0.42)] md:block">
        <div className="flex h-full flex-col p-4">
          <div className="mb-6 rounded-[22px] border border-white/10 bg-white/8 p-4">
            <p className="font-[var(--font-heading)] text-xs uppercase tracking-[0.28em] text-white/78">Control personal</p>
            <h1 className="mt-3 font-[var(--font-heading)] text-2xl font-semibold leading-tight">Finanzas con foco y contexto</h1>
            <p className="mt-2 text-sm text-white/88">Todo tu sistema financiero en un lateral claro y siempre accesible.</p>
          </div>
            ...
            {ITEMS.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <p className={`mt-1 text-xs ${active ? "text-white/88" : "text-white/60"}`}>{item.hint}</p>
                </Link>
              );
            })}
          </nav>
