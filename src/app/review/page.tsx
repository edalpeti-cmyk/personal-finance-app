...
export default function ReviewPage() {
  ...
  const reviewActions = useMemo<Array<ReviewAction & { completed: boolean }>>(() => {
    ...
    if (reviewMetrics.activeGoalsCount === 0) {
      actions.push({ id: "goals-empty", title: "Todavia no hay objetivos activos", body: "A�adir metas te ayuda a convertir la revision mensual en decisiones de ahorro reales.", href: "/goals", cta: "Crear objetivo", tone: "info" });
    }
  ...
  const reviewConclusion = useMemo<ReviewConclusion>(() => {
    ...
    if (reviewMetrics.actualSavings < reviewMetrics.currentSavingsTarget) {
      return {
        ...
        decision: "Mant�n el cierre, pero ajusta aportaciones, objetivos o gasto del proximo mes para recuperar tracci�n.",
        href: "/goals",
        cta: "Revisar metas"
      };
    }
          ...
          <>
            <section className="panel rounded-[28px] p-5 text-white xl:col-span-12">
              <SectionHeader
                ...
                description="La revision ya no es solo una pantalla de lectura: ahora te gu�a desde la base operativa hasta la perspectiva FIRE."
