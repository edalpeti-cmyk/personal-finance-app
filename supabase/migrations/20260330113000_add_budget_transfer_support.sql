...
check (budget_kind in ('expense', 'investment_transfer', 'emergency_fund'));

create table if not exists public.internal_transfers (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references public.users(id) on delete cascade,
  category text not null,
  transfer_type text not null default 'investment' check (transfer_type in ('investment', 'emergency_fund')),
